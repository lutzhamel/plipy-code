...
from cuppa3_output import output, output_data
...
def cc(input_stream,
    ...
    try:
        ...
        # wrap the instr stream appropriate
        # gnu assembler
        instr_stream = list()
        instr_stream += [('.text',)]
        instr_stream += [('.global','_start')]
        instr_stream += [('_start:',)]
        instr_stream += codegen(ast)
        instr_stream += [('call','exit')]
        ...
        bytecode = ''
        bytecode += output_data(symtab.global_vars)
